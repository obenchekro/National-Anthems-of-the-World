...
def create_dropdown(df, column_name):
...
#  %%
def distribution(df, specific_col=None):
    sns.set(style="whitegrid")  # Style de grille plus épuré
    ...
    if specific_col is None:
        ...
        def plot_distribution(col_name):
            _plot_distribution(df, col_name)

    else:
        _plot_distribution(df, specific_col)


def _plot_distribution(df, col_name):
    ...
    plt.title(
        f"Distribution de la colonne {col_name} ({col_type_str})",
        fontsize=16,
        fontweight="bold",
    )
...
def sns_plot(
    ...
    size_slider = widgets.IntSlider(
        value=10, min=4, max=20, step=1, description="Taille de la figure:"
    )
